...
export default function LanguageSwitcherLanding({
  className = '',
}: LanguageSwitcherLandingProps) {
  ...
  return (
    <PopoverPrimitive.Root open={open} onOpenChange={setOpen}>
      <PopoverPrimitive.Trigger asChild>
        ...
        >
          <span className="group-hover:naxatw-underline">
            {getLocaleDisplayName(current)}
          </span>
          <svg
            aria-hidden="true"
            width="12"
          ...
          {locales.map(locale => (
            <button
              ...
              className={`naxatw-block naxatw-w-full naxatw-cursor-pointer naxatw-px-4 naxatw-py-1.5 naxatw-text-left naxatw-text-body-sm hover:naxatw-bg-grey-100 ${
                locale === current
                  ? 'naxatw-bg-grey-100 naxatw-font-semibold'
                  : ''
              }`}
            >
