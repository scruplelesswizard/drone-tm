...
@router.get(
    "/mentionable",
    tags=["users"],
    response_model=user_schemas.MentionableUsersOut,
    summary="List every user's id/name/profile_img for the @-mention picker",
)
async def get_mentionable_users(
    db: Annotated[Connection, Depends(database.get_db)],
    user_data: Annotated[AuthUser, Depends(login_required)],
):
    """Unpaginated, field-minimal - decoupled from GET /users's real
    pagination so that endpoint isn't forced to serve "all users in one
    page" just for this picker's client-side filtering.
    """
    results = await user_schemas.DbUser.all_mentionable(db)
