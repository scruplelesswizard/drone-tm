...
async def test_request_id_is_echoed_back_when_provided(client):
    ...
    response = await client.get("/api/users/my-info", headers={"X-Request-ID": given})
...
async def test_request_id_differs_across_requests_when_not_supplied(client):
    ...
    cached one."""
    first = await client.get("/api/users/my-info")
    second = await client.get("/api/users/my-info")
